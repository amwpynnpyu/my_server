...
std::wstring ansi_to_unicode(const char* str)
{
	assert(str);
#if (defined(WIN32) || defined(WIN64))
	int textlen = MultiByteToWideChar(CP_ACP, 0, str, -1, NULL, 0);
	std::wstring w(textlen - 1, 0);
	MultiByteToWideChar(CP_ACP, 0, str, -1, &w[0], textlen - 1);
	return w;
#else
	return std::wstring();
#endif
...
std::string unicode_to_ansi(const wchar_t* str)
{
	assert(str);
#if (defined(WIN32) || defined(WIN64))
	int textlen = WideCharToMultiByte(CP_ACP, 0, str, -1, NULL, 0, NULL, NULL);
	std::string s(textlen - 1, 0);
	WideCharToMultiByte(CP_ACP, 0, str, -1, &s[0], textlen - 1, NULL, NULL);
	return s;
#else
	return std::string();
#endif
...
std::wstring utf8_to_unicode(const char* str)
{
	...
	std::wstring w(textlen - 1, 0);
	MultiByteToWideChar(CP_UTF8, 0, str, -1, &w[0], textlen - 1);
	return w;
#else
	return std::wstring();
#endif
...
std::string unicode_to_utf8(const wchar_t* str)
{
	...
	std::string s(textlen - 1, 0);
	WideCharToMultiByte(CP_UTF8, 0, str, -1, &s[0], textlen - 1, NULL, NULL);
	return s;
#else
	return std::string();
#endif
...
std::string utf8_to_ansi(const char* str)
{
	assert(str);
#if (defined(WIN32) || defined(WIN64))
	return unicode_to_ansi(utf8_to_unicode(str).c_str());
#else
	return std::string();
#endif
}
