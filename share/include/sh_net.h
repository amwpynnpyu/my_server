...
extern const char* GetNetErrorMessage(E_NET_ERROR errcode);
...
public:
    ...
    virtual INT32 RawSend(const char* pBuf,INT32 nLen)=0;

    // 断开连接
    virtual void  Disconnect(void) = 0;

    // 获得连接远端地址32位整数值
    virtual const UINT32  GetRemoteIP(void) = 0;

    // 获得连接远端地址IP格式的字符串地址
    virtual const char*  GetRemoteIPStr(void) = 0;

    // 连接远端地址端口
    virtual UINT16  GetRemotePort(void) = 0;

    // 获得连接本地地址32位整数值
    virtual const UINT32  GetLocalIP(void) = 0;

    // 获得连接本地地址IP格式的字符串地址
    virtual const char*  GetLocalIPStr(void) = 0;
...
public:
    ...
    virtual bool CheckIp(const char* pszIP)=0;
};
...
public:
    ...
    virtual bool SetRemoteAddress(const char* pszIP, UINT16 wPort)=0;

    // 设置本连接的网络数据的收发缓冲区长度
    // 如果对已经建立好的连接调用这个函数，则
    // 需要Stop这个监听后，重新Start成功后才会使用这里设置的缓冲区长度
    // 合法的设置返回true，否则返回false
    virtual bool SetDataBufferSize(UINT32 dwRecvBufSize, UINT32 dwSendBufSize)=0;
};
...
public:
    ...
    virtual void SetRemoteAddress(const char* pszIP, UINT16 wPort)=0;
    ...
    virtual void SetAutoReconectMilSecond(UINT32 dwAutoReconnectMilSecond = 0xffffffff)=0;
...
public:
    ...
    */
    virtual IConnector*  CreateConnector(ISession* pSession,
    ...
    */
    virtual IListener*  CreateListener(ISessionFactory* pSessionFactory,
                                       UINT32 dwRecvBufSize, 
                                       UINT32 dwSendBufSize,
                                       const char* pszIP, 
                                       UINT16 wPort, 
...
public:
	...
	// 传入一个有效的链接指针
	// 用户自行保存这个指针
    virtual void  OnEstablish(IConnection* poConnection) = 0;

    // 当连接断开，会回调该函数
	// 传入一个 NULL 来使用用户原来保存有效连接指针无效
	// 用户必须使用传入的 参数， 为原来保存有效连接指针赋值，使其无效
    virtual void  OnTerminate(IConnection* poConnection) = 0;
...
public:
    ...
    virtual void FreeSession(ISession* pSession) = 0;
};

/// 返回指向网络模块对象的指针
INetModule* CreateNetModule();
